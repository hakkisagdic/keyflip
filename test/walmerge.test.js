...
function buildWal(opts) {
  ...
  const parts = [header];
  let s1 = hs1,
    s2 = hs2; // running checksum seeds from the (computed) header checksum
...
function page(ps, fill) {
  return Buffer.alloc(ps, fill);
}
...
test('overlay returns a null-prototype map of the committed pages', function () {
  ...
  assert.ok(
    ov[1].every(function (b) {
      return b === 0x11;
    }),
  );
  assert.ok(
    ov[2].every(function (b) {
      return b === 0x22;
    }),
  );
});
...
test('a corrupt (bad-checksum) frame stops the scan; a bad header yields nothing', function () {
  const ps = 512;
  const corrupt = buildWal({ pageSize: ps, corruptFrame: 0, frames: [{ pgno: 1, dbSize: 1, data: page(ps, 0x5a) }] });
  assert.strictEqual(Object.keys(wm.overlay(corrupt, ps)).length, 0, 'first frame fails checksum -> empty');
  const badHdr = buildWal({
    pageSize: ps,
    badHeaderCksum: true,
    frames: [{ pgno: 1, dbSize: 1, data: page(ps, 0x5a) }],
  });
...
test('big-endian-checksum WAL (magic ...0683) is read correctly', function () {
  const ps = 1024;
  const wal = buildWal({ pageSize: ps, magic: 0x377f0683, frames: [{ pgno: 3, dbSize: 3, data: page(ps, 0xee) }] });
  const ov = wm.overlay(wal, ps);
...
test('overlay infers page size from the WAL header when the arg is missing/bogus', function () {
  ...
  assert.strictEqual(
    Object.keys(wm.overlay(wal, 512)).length,
    0,
    'a valid but WRONG page size fails the checksum -> empty',
  );
});
...
test('SECURITY: a WAL declaring a huge page number cannot force a giant allocation (DoS cap)', function () {
  ...
  assert.ok(
    merged.length <= cap,
    'output is bounded to the physical input size, not the declared page number (' + merged.length + ' <= ' + cap + ')',
  );
...
test('applyOverlay returns the DB unchanged when there is nothing to apply', function () {
  ...
  db.writeUInt16BE(ps, 16);
  assert.strictEqual(
    wm.applyOverlay(db, Buffer.from('not a wal at all, just junk')),
    db,
    'garbage WAL -> same DB buffer',
  );
