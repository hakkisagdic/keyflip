...
function blobExpiring() {
  return JSON.stringify({
    claudeAiOauth: { accessToken: 'OLD', refreshToken: 'RT-1', expiresAt: NOW + 60000, scopes: ['a'] },
  });
}
function blobFresh() {
...
function okFetch(calls) {
  return async function (url, opts) {
    ...
    return {
      ok: true,
      json: async function () {
        return { access_token: 'NEW', refresh_token: 'RT-2', expires_in: 3600, scope: 'x y' };
      },
    };
  };
}
...
test('refreshBlob returns null on HTTP failure or missing refresh token', async function () {
  const bad = await oauth.refreshBlob(blobExpiring(), {
