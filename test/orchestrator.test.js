...
test('runJob is BLOCKED by a policy deny rule for the selected account — nothing spawns', async function () {
  const ctx = makeCtx();
  ctx.store.setProfile(
    'work',
    JSON.stringify({ claudeAiOauth: { accessToken: 'TW', refreshToken: 'RW', expiresAt: 9999999999999 } }),
  );
  profiles.write(ctx.configDir, {
    name: 'work',
    email: 'w@x.com',
    oauthAccount: { emailAddress: 'w@x.com', organizationUuid: 'o' },
    savedAt: ctx.now(),
  });
  ...
  const rec = [];
  const done = await orch.runJob(ctx, job, {
...
function seed(ctx, name, email, token) {
  ctx.store.setProfile(
    name,
    JSON.stringify({ claudeAiOauth: { accessToken: token, refreshToken: 'R' + token, expiresAt: 9999999999999 } }),
  );
  profiles.write(ctx.configDir, {
    name: name,
    email: email,
...
function fakeFetch(pctByToken) {
  return async function (url, opts) {
    ...
    return {
      ...
      status: 200,
      json: async function () {
        return { five_hour: { utilization: pct, resets_at: null } };
      },
    };
  };
}
...
function fakeRun(record, opts) {
  ...
  return function (bin, args, input, o) {
    ...
    if (opts.failFor && dir && dir.indexOf(path.sep + opts.failFor) !== -1)
      return { code: 2, stdout: '', stderr: 'boom for ' + opts.failFor, error: null };
...
test('enqueue rejects an empty prompt and an invalid group', function () {
  ...
  assert.throws(function () {
    orch.enqueue(ctx, { prompt: '   ' });
  }, /non-empty prompt/);
  assert.throws(function () {
    orch.enqueue(ctx, {});
  }, /non-empty prompt/);
  assert.throws(function () {
    orch.enqueue(ctx, { prompt: 'x', group: '__proto__' });
  }, /invalid group/);
  assert.throws(function () {
    orch.enqueue(ctx, { prompt: 'x', group: '../evil' });
  }, /invalid group/);
});
...
test('a corrupt jobs.json is empty to readers but REFUSES to be clobbered on write', function () {
  ...
  assert.throws(function () {
    orch.enqueue(ctx, { prompt: 'x' });
  }, /not valid JSON/); // write path throws
});
...
test('fanOut runs the SAME prompt across accounts, each in its own isolated dir', async function () {
  ...
  seed(ctx, 'gamma', 'g@x.com', 'TG');
  const rec = [];
  const out = await orch.fanOut(ctx, 'same prompt', ['alpha', 'beta', 'gamma'], {
    run: fakeRun(rec, { failFor: 'beta' }),
    cwd: '/fan',
  });
  ...
  const dirs = rec.map(function (r) {
    return r.configDir;
  });
  assert.strictEqual(new Set(dirs).size, 3);
  rec.forEach(function (r) {
    assert.deepStrictEqual(r.args, ['-p', 'same prompt']);
    assert.strictEqual(r.cwd, '/fan');
  });
});
...
function tool(name) {
  return orch.mcpTools.filter(function (t) {
    return t.name === name;
  })[0];
}
...
test('MCP mutating tools throw without confirm=true (and never spawn)', async function () {
  const ctx = makeCtx();
  await assert.rejects(function () {
    return tool('keyflip_job_enqueue').run(ctx, { prompt: 'p' });
  }, /confirmation required/);
  await assert.rejects(function () {
    return tool('keyflip_job_run').run(ctx, {});
  }, /confirmation required/);
  await assert.rejects(function () {
    return tool('keyflip_fanout').run(ctx, { prompt: 'p', accounts: ['a'] });
  }, /confirmation required/);
  assert.strictEqual(orch.list(ctx).length, 0);
});
...
test('MCP keyflip_job_run rejects an unknown or non-queued id', async function () {
  const ctx = makeCtx();
  await assert.rejects(function () {
    return tool('keyflip_job_run').run(ctx, { id: 'nope', confirm: true });
  }, /no such job/);
  await assert.rejects(function () {
    return tool('keyflip_job_run').run(ctx, { confirm: true });
  }, /no queued jobs/);
});
