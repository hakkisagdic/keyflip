...
function sharedDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'kf-fleet-'));
}
...
function busOf(ctx, dir) {
  return fleet.bus(ctx, { dir: dir, passphrase: PASS });
}
...
test('account distribution C->B, orchestrated from A', function () {
  ...
  const cStatus = fleet.readFleet(A, busOf(A, dir)).find(function (s) {
    return s.name === 'gamma';
  });
  ...
  assert.strictEqual(
    B.store.getProfile('clientX'),
    '{"claudeAiOauth":{"accessToken":"CX-CRED"}}',
    "B now holds C's account",
  );
});
...
test('chat-status: a session whose last message is the assistant is "replied"', function () {
  const dir = sharedDir();
  const A = machine('alpha', dir);
  const pdir = path.join(A.claudeDir, 'projects', '-p');
  fs.mkdirSync(pdir, { recursive: true });
  fs.writeFileSync(
    path.join(pdir, 'sReplied.jsonl'),
    '{"type":"user","cwd":"/p","message":{"role":"user","content":"do X"}}\n' +
      '{"type":"assistant","message":{"role":"assistant","content":[{"type":"text","text":"done"}]}}\n',
  );
  fs.writeFileSync(
    path.join(pdir, 'sWaiting.jsonl'),
    '{"type":"user","cwd":"/p","message":{"role":"user","content":"are you there?"}}\n',
  );
  const st = fleet.buildStatus(A, {});
  const replied = st.chats.find(function (c) {
    return c.sessionId === 'sReplied';
  });
  const waiting = st.chats.find(function (c) {
    return c.sessionId === 'sWaiting';
  });
  ...
  const s1 = [
    {
      machineId: 'm1',
      name: 'beta',
      chats: [{ sessionId: 's', mtime: '2026-07-07T10:00:00Z', lastRole: 'user', replied: false, lastText: 'q' }],
    },
  ];
  ...
  const s2 = [
    {
      ...
      name: 'beta',
      chats: [
        { sessionId: 's', mtime: '2026-07-07T10:05:00Z', lastRole: 'assistant', replied: true, lastText: 'answer' },
      ],
    },
  ];
...
test('fleet.json does not pollute the account list (RESERVED)', function () {
  ...
  const names = _core.listProfiles(ctx).map(function (p) {
    return p.name;
  });
  assert.ok(
    names.indexOf('fleet') === -1 && names.indexOf('fleet-seen') === -1,
    'fleet.json/fleet-seen.json are not accounts',
  );
...
function writeRaw(dir, name, obj) {
  fs.writeFileSync(path.join(dir, name), sync.encrypt(JSON.stringify(obj), PASS), { mode: 0o600 });
}
...
test('P0 cred leak: readFleet keeps creds for relay, sanitizeStatus strips them for display', function () {
  ...
  assert.ok(
    raw.creds && raw.creds.work && raw.creds.work.cliCredentials.indexOf('TOP-SECRET') !== -1,
    'relay path still sees creds',
  );
...
test('P1 path traversal: a peer status claiming a ../ machineId is dropped by readFleet', function () {
  ...
  assert.ok(
    ...
    'traversal machineId never enters the roster',
  );
});
...
test('P1 path traversal: statusName/inboxName/queue reject an unsafe machine id', function () {
  ...
  assert.throws(function () {
    fleet.queue(A, busOf(A, dir), '../../evil', { type: 'note' });
  }, /invalid target/);
...
test('P1 spoofing: a status whose filename stem != claimed machineId is dropped (identity binding)', function () {
  ...
  assert.ok(
    !seen.some(function (s) {
      return s.name === 'impostor';
    }),
    'a machine cannot publish under another id',
  );
});
...
test('P2 RESERVED overwrite: an inbound save-account named "fleet" is rejected (cannot clobber fleet.json)', function () {
  ...
  const res = fleet.applyCommand(
    B,
    { type: 'save-account', payload: { account: { name: 'fleet', cliCredentials: '{"x":1}' } } },
    { allowSave: true },
  );
...
test('P2 replay: an already-applied command id is remembered and a stale command is rejected', function () {
  ...
  assert.strictEqual(
    fleet.commandFresh(ctx, { at: '2027-01-01T00:00:00Z' }),
    false,
    'a far-future command is rejected',
  );
});

test('robustness: normalizeStatus coerces hostile peer fields (non-array chats, control chars)', function () {
  const n = fleet.normalizeStatus({
    machineId: 'm1',
    name: 'ok\x1b[31mRED',
    chats: 'not-an-array',
    accounts: null,
...
test('robustness: newReplies never crashes on malformed statuses', function () {
  ...
  assert.doesNotThrow(function () {
    fleet.newReplies(ctx, [
      null,
      'str',
      { machineId: 'm', chats: 'nope' },
      { machineId: 'm2', chats: [null, { sessionId: 's', mtime: 1, replied: true }] },
    ]);
  });
});
...
test('origin-auth: a machine keypair is generated once and stable; the public key is base64', function () {
  ...
  const k1 = fleet.machineKeys(A),
    k2 = fleet.machineKeys(A);
...
test('origin-auth: a genuine signed command from a pinned peer passes checkOrigin end-to-end', function () {
  ...
  const cmd = fleet.queue(A, busOf(A, dir), fleet.identity(B).machineId, {
    type: 'switch',
    payload: { account: 'work' },
  });
...
test('origin-auth: a FORGED command (attacker key, claims from=alpha) is REJECTED even with the passphrase', function () {
  ...
  const A = machine('alpha', dir),
    B = machine('beta', dir),
    evil = machine('evil', dir);
  fleet.publish(A, busOf(A, dir), {}); // B will pin A's REAL key
  const forged = {
    id: 'deadbeef',
    from: fleet.identity(A).machineId,
    to: fleet.identity(B).machineId,
    at: A.now(),
    type: 'switch',
    payload: { account: 'work' },
  };
...
test('origin-auth: an UNSIGNED command is rejected', function () {
  const B = machine('beta', sharedDir());
  const origin = fleet.checkOrigin(
    B,
    { from: 'alpha-x', type: 'note', payload: {} },
    { keys: { 'alpha-x': 'AAA' }, conflicts: [] },
  );
...
test('origin-auth: applyCommand with requireSignature refuses a bad signature (defence in depth)', function () {
  ...
  const res = fleet.applyCommand(
    B,
    { type: 'switch', payload: { account: 'x' }, sig: 'bogus' },
    { allowSwitch: true, senderKey: fleet.publicKey(B), requireSignature: true },
  );
...
test('origin-auth: a pinned key that CHANGES is flagged as a conflict and its commands rejected until trusted', function () {
  ...
  const newKey = fleet.readFleet(B, busOf(B, dir)).find(function (s) {
    return s.name === 'alpha';
  }).pubKey;
...
test('origin-auth: a command signed FOR one machine is REJECTED when replayed into another inbox (recipient binding)', function () {
  ...
  assert.ok(
    !fleet.applyCommand(C, cmd, {
      allowSwitch: true,
      senderKey: recC.keys[fleet.identity(A).machineId],
      requireSignature: true,
    }).ok,
    'applyCommand also refuses the wrong recipient',
  );
...
test('origin-auth hardening: TOFU roster is prototype-pollution safe; fingerprint is stable hex', function () {
  const A = machine('alpha', sharedDir());
  const pub = fleet.publicKey(A);
  const rec = fleet.reconcileKeys(A, [
    { machineId: '__proto__', pubKey: pub },
    { machineId: 'constructor', pubKey: pub },
  ]);
...
test('P1 DNS-rebinding: the fleet panel rejects a non-loopback Host header', async function () {
  ...
  const A = machine('alpha', sharedDir());
  const panel = _panel;
  const h = await panel.serveFleet(A, {
    port: 0,
    getFleet: function () {
      return { machines: [], newReplies: [] };
    },
  });
  function get(hostHeader) {
    return new Promise(function (resolve) {
      const req = http.request(
        { host: '127.0.0.1', port: h.port, path: '/api/fleet', method: 'GET', headers: { host: hostHeader } },
        function (res) {
          res.resume();
