...
function sharedDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'kf-swarm-'));
}
...
test('applyExec is off by default: allowExec must be exactly true', function () {
  ...
  const cmd = {
    ...
    at: B.now(),
    type: 'exec',
    payload: { command: 'echo', args: ['y'] },
  };
  const run = fakeRun();
  assert.strictEqual(swarm.applyExec(B, cmd, { run: run }).skipped, 'consent');
  assert.strictEqual(
    swarm.applyExec(B, cmd, { allowExec: 1, run: run }).skipped,
    'consent',
    'truthy-but-not-true is refused',
  );
...
test('argv-array only: queueExec refuses a shell string and caps abuse', function () {
  ...
  assert.throws(function () {
    swarm.queueExec(A, busOf(A, dir), { command: 'sh', args: 'rm -rf', to: fleet.identity(A).machineId });
  }, /ARGV|array/i);
  assert.throws(function () {
    swarm.queueExec(A, busOf(A, dir), { command: '', to: fleet.identity(A).machineId });
  }, /command/i);
  const big = new Array(300).fill('x');
  assert.throws(function () {
    swarm.queueExec(A, busOf(A, dir), { command: 'echo', args: big, to: fleet.identity(A).machineId });
  }, /too many/i);
});
...
test('origin-auth: a FORGED exec (attacker key, claims from=alpha) is rejected and never runs, even with the passphrase', function () {
  ...
  const forged = {
    ...
    at: A.now(),
    type: 'exec',
    payload: { command: 'echo', args: ['pwned'], group: 'g' },
  };
...
test('non-exec commands are left in the inbox for `fleet push` (swarm only consumes exec)', function () {
  ...
  assert.ok(
    rest.some(function (c) {
      return c.type === 'switch';
    }),
    'switch is left for fleet push',
  );
  assert.ok(
    !rest.some(function (c) {
      return c.type === 'exec';
    }),
    'exec is consumed by the swarm drain',
  );
});
...
test("ping: queues a curl argv (no shell) for the operator's own url; rejects non-http / flag-like / whitespace urls", function () {
  ...
  assert.throws(function () {
    swarm.ping(A, busOf(A, dir), 'file:///etc/passwd', {});
  }, /URL/);
  assert.throws(
    function () {
      swarm.ping(A, busOf(A, dir), '-oremote', {});
    },
    /URL/,
    'a flag-like url cannot slip in as a curl option',
  );
  assert.throws(function () {
    swarm.ping(A, busOf(A, dir), 'http://a b/c', {});
  }, /URL/);
});
...
test('aggregate: only results addressed to this machine are visible; group filter scopes them', function () {
  ...
  swarm.drainExec(B, busOf(B, dir), {
    trustAll: true,
    allowExec: true,
    run: fakeRun(function () {
      return { code: 0, stdout: 'r1', stderr: '', error: null };
    }),
  });
  ...
  swarm.drainExec(B, busOf(B, dir), {
    ...
    allowExec: true,
    run: fakeRun(function () {
      return { code: 0, stdout: 'r2', stderr: '', error: null };
    }),
  });
  ...
  assert.ok(
    ...
    "C's result is not visible to A (addressed to C)",
  );
});
...
test('aggregate: a mislabelled result file (filename stem != claimed id) is ignored', function () {
  ...
  bA.write('spoof.result.enc', {
    id: 'other',
    type: 'exec-result',
    from: 'x',
    to: fleet.identity(A).machineId,
    at: A.now(),
    payload: { result: { ok: true, stdout: 'evil' } },
  });
  ...
  assert.ok(
    ...
    'a result file whose name does not match its id is dropped',
  );
});
...
test('aggregate: peer output is size-capped and control chars are scrubbed (tabs/newlines survive)', function () {
  ...
  swarm.drainExec(B, busOf(B, dir), {
    ...
    allowExec: true,
    run: fakeRun(function () {
      return { code: 0, stdout: big, stderr: '', error: null };
    }),
  });
...
test('applyExec caps output directly and reports a non-zero exit as result.ok=false while still "applied"', function () {
  ...
  const cmd = {
    ...
    at: B.now(),
    type: 'exec',
    payload: { command: 'false', args: [] },
  };
  const r = swarm.applyExec(B, cmd, {
    allowExec: true,
    unverified: true,
    run: function () {
      return { code: 3, stdout: 'A'.repeat(999999), stderr: 'boom', error: null };
    },
  });
...
test('robustness: aggregate id-filter is prototype-pollution safe', function () {
  ...
  assert.doesNotThrow(function () {
    swarm.aggregate(A, busOf(A, dir), { ids: ['__proto__', 'constructor', 'prototype'] });
  });
