  ...
  return new Promise(function (resolve, reject) {
    http
      .get(url, function (res) {
        ...
        res.on('end', function () {
          resolve({ status: res.statusCode, body: b, type: res.headers['content-type'], headers: res.headers });
        });
      })
      .on('error', reject);
  });
...
test('buildState aggregates accounts (+cached quota), providers, sessions, keepsakes', function () {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ alice: { at: 9e15, usage: { fiveHour: { pct: 40 }, sevenDay: { pct: 12 } } } }),
  );
...
test('G5: buildState attaches a per-account 5h trend from the usage history', function () {
  ...
  const lines = [10, 25, 40, 55].map(function (p) {
    return JSON.stringify({ at: ctx.now(), account: 'alice', status: 'ok', fiveHour: p, sevenDay: 5 });
  });
  fs.writeFileSync(path.join(ctx.configDir, 'usage-history.jsonl'), lines.join('\n') + '\n');
  const s = panel.buildState(ctx);
  const a = s.accounts.filter(function (x) {
    return x.name === 'alice';
  })[0];
...
test('serve responds with HTML on / and JSON on /api/state (loopback)', async function () {
  ...
  try {
    ...
    assert.ok(
      state.accounts.some(function (a) {
        return a.email === 'w@x.com';
      }),
    );
...
test('G5: buildActivity buckets sessions per UTC day, Sunday-aligned, window-scoped total', function () {
  ...
  const rows = [
    { mtime: '2026-07-07T09:00:00Z' },
    { mtime: '2026-07-07T23:00:00Z' }, // 2 today
  ...
  const jun30 = a.days.filter(function (d) {
    return d.date === '2026-06-30';
  })[0];
...
test('G5: buildMemoryGraph links keepsakes that share >=2 top terms', function () {
  ...
  const keyed = g.nodes.map(function (n) {
    return n.key;
  });
  const ai = keyed.indexOf('aaaa1111'),
    bi = keyed.indexOf('bbbb2222'),
    ci = keyed.indexOf('cccc3333');
  const linked = function (x, y) {
...
test('G8: buildSnapshot excludes all private content (sessions, keepsakes, memory graph)', function () {
  ...
  fs.writeFileSync(
    path.join(ctx.configDir, '.usage-cache.json'),
    JSON.stringify({ work: { at: 9e15, usage: { fiveHour: { pct: 55 } } } }),
  );
