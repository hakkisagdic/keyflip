...
test('parseGithub handles owner/repo, refs and subdirs', function () {
  assert.deepStrictEqual(skillstore.parseGithub('anthropics/skills'), {
    owner: 'anthropics',
    repo: 'skills',
    ref: 'HEAD',
    subdir: '',
  ...
  assert.strictEqual(g.ref, 'v1');
  assert.strictEqual(g.subdir, 'path/to/skill');
...
test("remove only removes keyflip-installed skills (never the user's own)", async function () {
  ...
  fs.writeFileSync(path.join(own, 'SKILL.md'), 'x');
  assert.throws(function () {
    skillstore.remove(ctx, 'mine');
  }, /not installed by keyflip/);
  assert.ok(fs.existsSync(own)); // untouched

  const src = path.join(ctx.home, 'pack');
  makeSkillDir(src, 'installed');
...
test('add refuses to clobber a pre-existing non-keyflip skill without --force', async function () {
  ...
  fs.writeFileSync(path.join(clash, 'SKILL.md'), 'USER');
  const src = path.join(ctx.home, 'pack');
  makeSkillDir(src, 'alpha');
  await assert.rejects(function () {
    return skillstore.add(ctx, src, {});
  }, /already exists/);
...
test('a GitHub subdir with ../ is rejected (path traversal)', async function () {
  ...
  const inner = path.join(staging, 'repo-main');
  ...
  await assert.rejects(function () {
    return skillstore.add(ctx, 'owner/repo/../../../../etc', { fetch: fetchMock });
  }, /traversal/);
});
