...
function tmpProject() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'kf-rules-'));
}
...
test('detectRuleFiles: reports every tool that has a rule file present, and nothing on an empty repo', function () {
  ...
  const cursor = detected.filter(function (d) {
    return d.tool === 'cursor';
  })[0];
  assert.ok(
    cursor.files.indexOf('.cursorrules') !== -1 &&
      cursor.files.indexOf(path.join('.cursor', 'rules', 'style.mdc')) !== -1,
    'both the flat file and the split rule dir are found',
  );
  assert.ok(
    cursor.label && typeof cursor.label === 'string',
    'tool carries a human label (reused from agents.REGISTRY)',
  );
});
...
test('importRules: builds the common model with provenance and per-file redaction counts', function () {
  ...
  assert.ok(
    model.sections.every(function (s) {
      return s.from === 'claude' || s.from === 'agents';
    }),
  );
  const kindsFrom = function (tool, heading) {
    return model.sections.filter(function (s) {
      return s.from === tool && s.heading === heading;
    })[0];
  };
...
test('classify: heuristic maps headings/bodies to the right kind, general as fallback', function () {
  ...
  assert.strictEqual(
    rules.classify('the service is split into a data model and component layers', 'Architecture'),
    'architecture',
  );
  assert.strictEqual(
    rules.classify('squash commits, open a pull request, wait for code review', 'Git Workflow'),
    'workflow',
  );
...
test('emit: renders each target with its title and groups sections by kind', function () {
  ...
  ['claude', 'cursor', 'agents', 'gemini', 'generic'].forEach(function (target) {
    ...
    assert.ok(
      content.indexOf('## Coding & Style') !== -1 && content.indexOf('## Security') !== -1,
      target + ' groups by kind',
    );
...
test('redactText: token shapes and credential-keyed lines redacted; env refs/placeholders kept', function () {
  ...
  assert.strictEqual(
    r.text.indexOf('hunter2plaintext'),
    -1,
    'credential-keyed line redacted even without a token shape',
  );
...
test('writeTarget: writes the target file inside the project and refuses to escape', function () {
  ...
  try {
    ...
    assert.throws(function () {
      rules.writeTarget(base, 'gemini', 'x');
    }, /refusing to write outside/);
    assert.ok(!fs.existsSync(outside), 'symlink target was never written through');
  } catch (e) {
    if (e.code !== 'EPERM' && e.code !== 'EACCES' && !/refusing/.test(e.message)) throw e;
  }
  assert.throws(function () {
    rules.writeTarget(base, 'bogus', 'x');
  }, /unknown emit target/);
});

test('assertModel: rejects malformed models', function () {
  assert.throws(function () {
    rules.assertModel(null);
  }, /not an object/);
  assert.throws(function () {
    rules.assertModel({});
  }, /sections must be an array/);
  assert.throws(function () {
    rules.assertModel({ sections: [{ from: 'x' }] });
  }, /no text string/);
