...
function stem(w) {
  ...
  if (w.length > 5 && w.slice(-3) === 'ing')
    w = w.slice(0, -3); // "limiting"->"limit"
  else if (w.length > 4 && w.slice(-2) === 'ed')
    w = w.slice(0, -2); // "limited"->"limit"
  else if (w.length > 3 && w.slice(-2) === 'es')
    w = w.slice(0, -2); // "caches"->"cach", "boxes"->"box"
...
function tokenize(s) {
  return String(s || '')
    ...
    .split(/[^a-z0-9]+/)
    .filter(function (t) {
      return t && t.length > 1 && !STOP.has(t);
    })
    .map(stem);
}
...
function corpus(ctx) {
  return memory
    ...
    .map(function (m) {
      ...
      return {
        key: m.key,
        session: sm ? sm[2].trim() : m.key,
        cwd: cm ? cm[2].trim() : null,
        text: text,
        toks: tokenize(text),
      };
    })
    .filter(function (d) {
      return d.toks.length;
    });
}
...
function snippet(text, terms) {
  ...
  const start = Math.max(0, at - 50),
    end = Math.min(text.length, at + 110);
...
function rank(docs, query, limit) {
  ...
  const N = docs.length;
  const avgdl =
    docs.reduce(function (s, d) {
      return s + d.toks.length;
    }, 0) / N;
  const df = Object.create(null);
  docs.forEach(function (d) {
    const seen = Object.create(null);
    d.toks.forEach(function (t) {
      if (!seen[t]) {
        seen[t] = 1;
        df[t] = (df[t] || 0) + 1;
      }
...
function answer(ctx, query, opts) {
  ...
  const context = hits
    ...
    .join('\n\n');
  const instruction =
    'Answer the QUESTION using ONLY the SOURCES that follow on stdin (each is a distilled memory of a past coding chat). Cite the session id in brackets like [' +
    hits[0].session.slice(0, 8) +
    '] after each supported claim. If the sources do not answer it, say so plainly. Be concise.\n\nQUESTION: ' +
    query +
    '\n\nSOURCES follow:';
...
async function semanticSearch(ctx, query, opts) {
  ...
  docs.forEach(function (d, i) {
    ...
    const c = cache[d.key];
    if (!c || c.hash !== d._hash || c.space !== space) {
      need.push(d.text);
      needIdx.push(i);
    }
  });
  try {
    if (need.length) {
      ...
      needIdx.forEach(function (di, k) {
        if (vecs[k]) cache[docs[di].key] = { hash: docs[di]._hash, space: space, vec: vecs[k] };
      });
      try {
    ...
    return { ok: false, reason: (e && e.message) || 'embed-failed', hits: [] };
  }
}
