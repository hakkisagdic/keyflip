...
function cookiesPath(ctx) {
  return ctx.appDataDir ? path.join(ctx.appDataDir, 'Cookies') : null;
}
function cookiesJournalPath(ctx) {
  return ctx.appDataDir ? path.join(ctx.appDataDir, 'Cookies-journal') : null;
}
function profileCookiesPath(ctx, name) {
  return path.join(ctx.configDir, 'app', name + '.cookies');
}

function configPath(ctx) {
  return ctx.appDataDir ? path.join(ctx.appDataDir, 'config.json') : null;
}
// Kept in an "app/" subdir so it never shows up in profiles.list() (which scans
// configDir for <name>.json).
function profilePath(ctx, name) {
  return path.join(ctx.configDir, 'app', name + '.json');
}
function readJSON(p) {
  try {
...
function hasProfile(ctx, name) {
  try {
    return fs.existsSync(profilePath(ctx, name));
  } catch (e) {
    return false;
  }
}
...
function detectActiveOrg(ctx) {
  ...
  Object.keys(cfg).forEach(function (k) {
    ...
    if (m && typeof cfg[k] === 'string' && cfg[k] > bestTs) {
      bestTs = cfg[k];
      best = m[1];
    }
  });
...
function isV10(b64) {
  try {
    return Buffer.from(String(b64), 'base64').slice(0, 3).toString() === 'v10';
  } catch (e) {
    return false;
  }
}
...
function isEncBlob(b64) {
  try {
    const p = Buffer.from(String(b64), 'base64').slice(0, 3).toString();
    return p === 'v10' || p === 'v11';
  } catch (e) {
    return false;
  }
...
function decryptAppBlobWin(ctx, b64) {
  ...
  let ls;
  try {
    ls = fs.readFileSync(lsPath, 'utf8');
  } catch (e) {
    return null;
  }
...
function emailForAccount(ctx, account) {
  ...
  return (
    findEmailUnder(path.join(ctx.appDataDir, 'local-agent-mode-sessions', account), 80) ||
...
function detectAppAccount(ctx) {
  ...
  while ((m = re.exec(text)) !== null) {
    if (uuids.indexOf(m[0]) === -1) uuids.push(m[0]);
  }
  ...
  uuids.forEach(function (u) {
    if (!org && cfg['dxt:allowlistLastUpdated:' + u] !== undefined) org = u;
  });
  const accts = (function () {
    try {
  ...
  accts.forEach(function (a) {
    ...
    orgs.forEach(function (o) {
      if (org ? o === org : uuids.indexOf(o) !== -1) {
        org = org || o;
  ...
  return {
    ...
    email: emailForAccount(ctx, account),
    reason: org ? null : 'unresolved-org',
  };
}
...
function cookiesLookLoggedIn(file) {
  try {
    return fs.readFileSync(file).includes('sessionKey');
  } catch (e) {
    return false;
  }
}
...
function snapshotToProfile(ctx, name) {
  ...
  try {
    ...
    /* cookies snapshot is best-effort */
  }
...
function applyFromProfile(ctx, name) {
  ...
  let mode = 0o600;
  try {
    mode = fs.statSync(cp).mode & 0o777;
  } catch (e) {
    /* keep default */
  }
  ...
  try {
    ...
    if (ck && fs.existsSync(savedCk)) {
      ...
      try {
        fs.rmSync(cookiesJournalPath(ctx), { force: true });
      } catch (e) {
        /* stale journal */
      }
    }
