...
function jobsPath(ctx) {
  return path.join(ctx.configDir, 'jobs.json');
}
...
function readStateSafe(ctx) {
  try {
    return normalizeState(readJsonForWrite(jobsPath(ctx)));
  } catch (e) {
    return { jobs: [] };
  }
}
...
function readStateForWrite(ctx) {
  return normalizeState(readJsonForWrite(jobsPath(ctx)));
}
...
function updateJob(ctx, id, patch) {
  ...
  let found = null;
  state.jobs.forEach(function (jb) {
    if (jb && jb.id === id) {
      Object.assign(jb, patch);
      found = jb;
    }
  });
...
function finish(ctx, job, patch) {
  return applyPatch(ctx, job, Object.assign({ finishedAt: ctx.now() }, patch));
}
...
function list(ctx) {
  return readStateSafe(ctx).jobs;
}
function get(ctx, id) {
  if (id == null) return null;
  const want = String(id);
  return (
    readStateSafe(ctx).jobs.filter(function (j) {
      return j.id === want;
    })[0] || null
  );
}
...
async function selectAccount(ctx, spec, opts) {
  ...
  const candidates = profs.filter(function (p) {
    try {
      return !!ctx.store.getProfile(p.name);
    } catch (e) {
      return false;
    }
  });
...
function runResultError(res) {
  return (
    String(
      (res && (res.stderr || (res.error && res.error.message))) || 'claude exited with code ' + (res && res.code),
    ).trim() || 'run failed'
  );
}
...
async function runJob(ctx, job, opts) {
  ...
  let name;
  try {
    name = await selectAccount(ctx, { group: job.group, strategy: opts.strategy }, opts);
  } catch (e) {
    return finish(ctx, job, { status: 'error', error: (e && e.message) || 'account selection failed' });
  }
  if (!name)
    return finish(ctx, job, {
      status: 'error',
      error: 'no available account' + (job.group ? " in group '" + job.group + "'" : ''),
    });
  ...
  let res;
  try {
    res = runAs(ctx, name, job.prompt, job.cwd, opts);
  } catch (e) {
    return finish(ctx, job, { status: 'error', account: name, error: (e && e.message) || 'run failed' });
  }
  if (!res || res.code !== 0)
    return finish(ctx, job, { status: 'error', account: name, error: cap(runResultError(res)) });
...
async function runNext(ctx, opts) {
  const next = readStateSafe(ctx).jobs.filter(function (j) {
...
async function fanOut(ctx, prompt, accountNames, opts) {
  ...
  for (let i = 0; i < names.length; i++) {
    ...
    if (!pol.allowed) {
      entry.error = 'policy denied: ' + pol.reason;
      out.push(entry);
      continue;
    }
    try {
      ...
      entry.error = (e && e.message) || 'run failed';
    }
    out.push(entry);
  }
  return out;
...
const mcpTools = [
  {
    ...
    description:
      'List headless jobs (id, status, chosen account, prompt, captured result/error). Optionally filter by status, or fetch one by id. Read-only.',
    inputSchema: {
      type: 'object',
      properties: {
        id: { type: 'string', description: 'Fetch a single job by id.' },
        status: { type: 'string', enum: STATUSES, description: 'Filter to one status.' },
      },
  ...
  },
  {
    ...
    description:
      'Add a prompt to the headless job queue to later run on the best available account (by quota headroom), optionally scoped to a group. Does NOT run it — use keyflip_job_run. Mutating (writes the queue) — ask the user, then confirm=true.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        strategy: {
          ...
          description: 'Account selection strategy (default best).',
        },
        confirm: mcpConfirm,
      },
      required: ['confirm'],
    ...
    run: async function (ctx, args) {
      ...
      if (args.id) {
        ...
        if (job.status !== 'queued')
          throw new Error("job '" + args.id + "' is not queued (status: " + job.status + ')');
  ...
  },
  {
    ...
    description:
      "Run the SAME prompt HEADLESS across several saved accounts at once (each in its own ISOLATED CLAUDE_CONFIG_DIR), returning each account's output. IMPORTANT: this SPENDS quota on EVERY listed account. Ask the user, then confirm=true.",
    inputSchema: {
      type: 'object',
      properties: {
