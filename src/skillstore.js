...
function skillsDir(ctx) {
  return path.join(ctx.claudeDir || path.join(ctx.home, '.claude'), 'skills');
}
function manifestPath(ctx) {
  return path.join(ctx.configDir, 'installed-skills.json');
}

function readManifest(ctx) {
  try {
    const o = JSON.parse(fs.readFileSync(manifestPath(ctx), 'utf8'));
    return o && typeof o === 'object' ? o : {};
  } catch (e) {
    return {};
  }
}
function writeManifest(ctx, m) {
  writeJsonStable(manifestPath(ctx), m, 0o600);
}
  ...
  function walk(dir, d) {
    ...
    if (
      entries.some(function (e) {
        return e.isFile() && e.name === 'SKILL.md';
      })
    ) {
      found.push(dir);
      return;
    } // a skill dir isn't nested
    if (d <= 0) return;
    ...
    entries.forEach(function (e) {
      if (e.isDirectory() && e.name[0] !== '.') walk(path.join(dir, e.name), d - 1);
    });
  }
  walk(root, depth);
  return found;
...
function installFromDir(ctx, srcRoot, source, opts) {
  ...
  dirs.forEach(function (d) {
    ...
    if (opts.link) {
      try {
        fs.symlinkSync(fs.realpathSync(d), target, 'dir');
        mode = 'symlink';
      } catch (e) {
        fs.cpSync(d, target, { recursive: true });
      }
    } else {
      fs.cpSync(d, target, { recursive: true });
    }
...
function extractArchive(file) {
  ...
  if (r.code !== 0) {
    ...
    throw new Error('could not extract ' + file + ' (need tar): ' + (r.stderr || r.code));
  }
  return tmp;
}
...
async function resolveSource(src, opts) {
  ...
  // local directory
  try {
    if (fs.statSync(src).isDirectory()) return { root: src, cleanup: null, source: 'dir:' + path.resolve(src) };
  } catch (e) {
    /* not a dir */
  }
  // local archive
  ...
  if (gh) {
    ...
    if (!res || !res.ok)
      throw new Error('GitHub download failed (http ' + (res && res.status) + ') for ' + gh.owner + '/' + gh.repo);
    ...
    let inner = dir;
    try {
      const only = fs.readdirSync(dir).filter(function (n) {
        return fs.statSync(path.join(dir, n)).isDirectory();
      });
      if (only.length === 1) inner = path.join(dir, only[0]);
    } catch (e) {
      /* */
    }
    if (gh.subdir) {
    ...
    return {
      root: inner,
      cleanup: dir,
      source: 'github:' + gh.owner + '/' + gh.repo + (gh.subdir ? '/' + gh.subdir : ''),
    };
  }
...
function list(ctx) {
  ...
  return Object.keys(m)
    .sort()
    .map(function (n) {
      return Object.assign({ name: n }, m[n]);
    });
}
...
function remove(ctx, name) {
  ...
  try {
    fs.rmSync(path.join(skillsDir(ctx), name), { recursive: true, force: true });
  } catch (e) {
    /* */
  }
  delete m[name];
