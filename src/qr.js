...
function buildCodewords(bytes, level, version) {
  ...
  ecb.groups.forEach(function (grp) {
    for (let b = 0; b < grp[0]; b++) {
      const d = data.slice(idx, idx + grp[1]);
      idx += grp[1];
...
function toText(qr, opts) {
  ...
  const at = function (r, c) {
    return r >= 0 && r < n && c >= 0 && c < n ? m[r][c] : 0;
  };
  const D = opts.invert ? ' ' : '',
    L = opts.invert ? '' : ' ';
  const lines = [];
  for (let r = -q; r < n + q; r += 2) {
    ...
    for (let c = -q; c < n + q; c++) {
      const top = at(r, c),
        bot = r + 1 < n + q ? at(r + 1, c) : 0;
      // in a light-background terminal a dark module should be a filled block
      const t = opts.invert ? !top : top,
        b = opts.invert ? !bot : bot;
      line += t && b ? '█' : t ? '▀' : b ? '▄' : ' ';
    }
    lines.push(line);
  }
