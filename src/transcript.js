...
function parse(jsonl) {
  ...
  const counts = {
    ...
    user: messages.filter(function (m) {
      return m.role === 'user';
    }).length,
    assistant: messages.filter(function (m) {
...
function toMarkdown(parsed, opts) {
  ...
  meta.push(
    parsed.counts.messages + ' messages (' + parsed.counts.user + ' you / ' + parsed.counts.assistant + ' Claude)',
  );
...
function toHtml(parsed, opts) {
  ...
  const bubbles = parsed.messages
    .map(function (m) {
      ...
      return (
        ...
        '"><div class="who">' +
        esc(who(m.role)) +
        (m.ts ? ' <span class="ts">' + esc(shortTs(m.ts)) + '</span>' : '') +
        '</div>' +
        body +
        tools +
        '</div>'
  ...
  return (
    ...
    '<meta name="viewport" content="width=device-width,initial-scale=1"><title>keyflip session ' +
    esc((opts.id || '').slice(0, 8)) +
    '</title>' +
    '<style>' +
    HTML_STYLE +
    '</style></head><body>' +
    '<header><h1>⚡ keyflip <span class="dim">session ' +
    esc((opts.id || '').slice(0, 8)) +
    '</span></h1><div class="dim">' +
    sub +
    '</div></header>' +
    '<main>' +
    (bubbles || '<p class="dim">No messages.</p>') +
    '</main>' +
