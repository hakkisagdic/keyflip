...
function defaultUrl() {
  return process.env.KEYFLIP_EMBED_URL || 'http://localhost:11434/api/embed';
}
function defaultModel() {
  return process.env.KEYFLIP_EMBED_MODEL || 'nomic-embed-text';
}
...
function postJson(url, body, timeoutMs) {
  return new Promise(function (resolve, reject) {
    ...
    try {
      ...
      return reject(new Error('bad embed URL: ' + url));
    }
    ...
    const req = lib.request(
      ...
      function (res) {
        ...
        res.on('data', function (d) {
          b += d;
          if (b.length > 64 * 1024 * 1024) req.destroy(new Error('embed response too large'));
        });
        res.on('end', function () {
          ...
          try {
            ...
            reject(new Error('embed endpoint returned invalid JSON'));
          }
        });
      },
    );
    req.on('timeout', function () {
      req.destroy(new Error('embed endpoint timed out'));
    });
    req.on('error', function (e) {
      reject(
        new Error(
          'embed endpoint unreachable (' + ((e && e.message) || 'error') + ') — is Ollama running? try `ollama serve`',
        ),
      );
    });
    req.end(payload);
...
async function embed(texts, opts) {
  ...
  if (res && Array.isArray(res.data)) {
    ...
    res.data.forEach(function (d, i) {
      out[d && typeof d.index === 'number' ? d.index : i] = d && d.embedding;
    });
    return out;
  }
