...
function scrubVisible(obj, scrub) {
  ...
  if (m && typeof m.content === 'string') {
    ...
    m.content.forEach(function (b) {
      ...
      else if (b && b.type === 'thinking' && typeof b.thinking === 'string') {
        scanned = true;
        b.thinking = apply(b.thinking);
      }
    });
  }
  if (obj && typeof obj.text === 'string') {
    scanned = true;
    obj.text = apply(obj.text);
  }
  if (obj && typeof obj.summary === 'string') {
    scanned = true;
    obj.summary = apply(obj.summary);
  }
...
function deleteSession(ctx, opts) {
  ...
  if (opts.hard === true) {
    ...
    try {
      ...
      return { ok: false, reason: 'delete-failed', detail: e.message };
    }
...
function scrubSession(ctx, opts) {
  ...
  let bak;
  try {
    bak = backup(loc.file, raw);
  } catch (e) {
    return { ok: false, reason: 'backup-failed', detail: e.message };
  }
  ...
  try {
    fsutil.atomicWrite(loc.file, outLines.join('\n'), 0o600);
  } catch (e) {
...
function editSession(ctx, opts) {
  ...
  if (type === 'delete-message') {
    newLines = events.filter(function (_, k) {
      return k !== index;
    });
    ...
    try {
      newLines[index] = JSON.stringify(obj);
    } catch (e) {
  ...
  try {
    fsutil.atomicWrite(loc.file, joinLines(newLines), 0o600);
  } catch (e) {
    return { ok: false, reason: 'write-failed', detail: e.message };
  }
