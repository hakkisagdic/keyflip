...
function store(ctx) {
  return path.join(ctx.configDir, 'archive');
}
...
function archiveSession(ctx, project, sessionId) {
  ...
  try {
    ...
    /* keep the archive even if the original can't be removed */
  }
...
function unarchiveSession(ctx, project, sessionId) {
  ...
  let comp;
  try {
    comp = fs.readFileSync(gz);
  } catch (e) {
    return { ok: false, reason: 'not-archived' };
  }
  let raw;
  try {
    raw = zlib.gunzipSync(comp);
  } catch (e) {
...
function listArchived(ctx) {
  ...
  projs.forEach(function (p) {
    let files;
    try {
      files = fs.readdirSync(path.join(root, p));
    } catch (e) {
      return;
    }
    files.forEach(function (f) {
      ...
      let st;
      try {
        st = fs.statSync(path.join(root, p, f));
      } catch (e) {
        return;
      }
      out.push({
        project: p,
        sessionId: f.slice(0, -9),
        gzBytes: st.size,
        mtimeMs: st.mtimeMs,
...
function findArchived(ctx, idOrPrefix) {
  ...
  const pre = all.filter(function (r) {
