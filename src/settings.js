...
function isCredentialKey(k) {
  ...
  return (
    /(_API_KEY|_AUTH_TOKEN|SECRET|TOKEN|_KEY|APIKEY|AUTHTOKEN|PASSWORD|PASSWD)$/i.test(s) ||
    /SECRET|PASSWORD|APIKEY|API_KEY/i.test(s) ||
    /(?:^|[^A-Z])(apiKey|authToken|accessToken|secretKey)$/.test(s)
  );
}
...
function stripCredentialEnv(env) {
  ...
  Object.keys(env || {}).forEach(function (k) {
    if (!isCredentialKey(k)) out[k] = env[k];
  });
  return out;
}
...
function setPath(obj, kp, val) {
  ...
  for (let i = 0; i < parts.length - 1; i++) {
    const p = parts[i];
    if (PROTO_KEYS.indexOf(p) !== -1) return;
    if (!o[p] || typeof o[p] !== 'object' || Array.isArray(o[p])) o[p] = {};
    o = o[p];
  }
  const last = parts[parts.length - 1];
  if (PROTO_KEYS.indexOf(last) !== -1) return;
